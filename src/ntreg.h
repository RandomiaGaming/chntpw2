...
#define HBIN_PAGESIZE 0x1000	 // hbin page size. hbins are minimum this, and always multiple of this
#define REGF_FILEDIVISOR 0x40000 // Hive filesize seems to always be multiple of this
#define VAL_DIRECT_LIMIT 0x3fd0	 // Larger than this, and values seems split into several blocks
...
struct lf_key
{
	...
	union
	{
		...
		// WinXP uses a more real hash instead (base 37 of uppercase name chars) padded with 0's. Case sensitive!
		struct lh_hash
		{
			int32_t ofs_nk; // 0x0000 D-Word Offset of corresponding "nk"-Record
...
struct vk_key
{
	...
	short flag;		  // 0x0010 Word Flag or 0x0001 ANSI encoding
...
struct hive
{
	...
	int type;		   // Suggested type of hive. NOTE: Library will guess when it loads it, but application may change it if needed
...
struct keyval *get_val2buf(struct hive *hdesc, struct keyval *kv, int vofs, char *path, int type, int exact);
...
int put_buf2val(struct hive *hdesc, struct keyval *kv, int vofs, char *path, int type, int exact);
